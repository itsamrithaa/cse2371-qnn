...
from typing import Sequence # For type hints.
...
def plot_loss_curve(hist: dict) -> None:
    ...
    plt.figure()  
    plt.plot(hist["epoch"], hist["train_loss"], label="train loss")  # Training loss line.
    plt.plot(hist["epoch"], hist["val_rmse"], label="val RMSE")  # Validation RMSE line.
    plt.xlabel("epoch")  
...
def residual_hist(residuals: Sequence[float], bins: int = 40, title: str = "Residuals") -> None:
    ...
    plt.figure()  
    plt.hist(residuals, bins=bins)  # Histogram of errors.
